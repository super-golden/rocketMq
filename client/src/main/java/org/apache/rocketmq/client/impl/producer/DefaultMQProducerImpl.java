...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    private SendResult sendDefaultImpl(
         ...
         * 如果没有缓存或者没有包含消息队列，则向NameServer查询topic的路由信息。如果最终未找到路由信息
         * ，则抛出异常
         */
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                //如果消息体默认超过4k，会对消息体采用zip压缩，并设置标识
                 ...
                 * 构建消息发送包
                 * 主要包含如下重要消息
                 *  生产者组，主题名称，默认创建主题KEy，该主题在单个Broker默认队列数，队列ID，消息系统标记（MessageSysFlag）
                 *  消息发送时间，消息标记（RocketMQ对消息中的flag不做任何处理，供应用程序使用）
                 *  消息拓展属性，消息重试次数，是否是批量消息
                 */
