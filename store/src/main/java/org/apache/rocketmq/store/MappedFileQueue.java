...
public class MappedFileQueue {
     ...
     * 从MappedFile列表中第一个文件开始查找，找到第一个最后更新时间大于待查找时间戳的文件，如果不存在，则返回最后一个MappedFile文件
     * @param timestamp 待查找文件的时间戳
     * @return
     */
    ...
    /**
     * 加载Commitlog文件，加载${ROCKET_HOME}/store/commitlog目录下所有文件并按照文件名排序。如果文件大小与配置的单个文件大小不一致，将忽略该目录下所有的文件
     * ，然后创建MappedFile对象。注意load方法将wortePosition、flushedPosition
     * committedPosition
     * 三个指针都设置为文件大小
     * @return
     */
    ...
    public int deleteExpiredFileByTime(final long expiredTime,
        ...
        int mfsLength = mfs.length - 1;//倒数第二个文件
        int deleteCount = 0;
        List<MappedFile> files = new ArrayList<MappedFile>();
        if (null != mfs) {
            for (int i = 0; i < mfsLength; i++) {
                MappedFile mappedFile = (MappedFile) mfs[i];
                //计算文件的最大存活时间（=文件最后一次更新时间+文件存活时间（默认72小时））
                long liveMaxTimestamp = mappedFile.getLastModifiedTimestamp() + expiredTime;
                if (System.currentTimeMillis() >= liveMaxTimestamp || cleanImmediately) {
                    //清除MappedFile占有的相关资源，如果执行成功，将文件加入到待删除文件列表中，然后统一执行File#delete方法将文件从物理磁盘中删除
