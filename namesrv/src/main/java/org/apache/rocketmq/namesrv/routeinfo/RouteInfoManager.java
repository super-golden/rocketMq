...
public class RouteInfoManager {
    ...
    //Broker集群消息，存储集群中所有的Broker名称
    private final HashMap<String/* clusterName */, Set<String/* brokerName */>> clusterAddrTable;
    //Broker状态信息，NameServer每次收到心跳包后会替换该消息
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                /**
                 * 路由注册需要加写锁，防止并发修改RouteInfoManager中的路由表。首先判断Broker所属集群是否存在，如果不存在，则创建
                 * ，然后将broker名加入到集群Broker集合中
                 */
                this.lock.writeLock().lockInterruptibly();
                ...
                /**
                 * 维护BrokerData信息，首先从brokerAddrTable根据BrokerName尝试获取Broker信息，如果不存在，则新建BrokerData并放入到
                 * brokerAddrTable，registerFirst 设置为true。如果存在，直接替换原先的，registerFirst 设置为false，表示非第一次注册
                 */
                 ...
                 */
                /**
                 * 如果Broker为Master，并且Broker Topic 配置信息发生改变或者时初次注册，则需要创建或更新Topic路由元数据，填充
                 * topicQueueTable，其实就是为默认主题自动注册路由信息，其中包含MixAll.DEFAULT_TOPIC的路由信息。当信息发送主题时
                 * 如果该主题未创建并且BrokerConfig的autoCreateTopicEnable为true时，将返回MixAll.DEFAULT_TOPIC
                 * 的路由信息
                 */
                ...
                /**
                 * 更新BrokerLiveInfo，存活Broker信息表，BrokerLiveInfo是执行删除的重要依据
                 */
